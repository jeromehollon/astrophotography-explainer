...
import { ROITile, Eyebrow } from './ui';
...
function Bar({ label, value, biasShare }: { label: string; value: number; biasShare: boolean }) {
  return (
    <div className="flex items-center gap-[16px]">
      <span className="w-[96px] t-label-md text-text-on-stage">{label}</span>
      ...
      <span className="t-mono-lg whitespace-nowrap text-text-on-stage">{fmt(value)}</span>
    </div>
  );
}
    ...
    <LessonPage>
      <div className="flex w-full flex-col items-start gap-[32px] px-[120px] py-[64px]">
        <div className="flex flex-col gap-[12px]">
          ...
          <h1 className="w-[900px] m-0 t-h1 tracking-[-0.5px] text-text-primary">Removing the bias from every pixel</h1>
          <p className="w-[680px] m-0 t-body-lg text-text-secondary">
        ...
        <section className="flex w-[1200px] flex-col gap-[16px] rounded-md border border-border-default bg-surface-card p-[24px]">
          ...
          <div className="flex w-[1152px] flex-col gap-[24px] t-body-md text-text-primary">
            <p>
        ...
        <section className="flex w-[1200px] flex-col gap-[24px]">
          <h2 className="w-[680px] m-0 t-h2 text-black">What the master bias looks like</h2>
          <p className="w-[680px] t-body-md text-black">
        ...
        <section className="flex w-[1168px] items-start gap-[48px] text-black">
          <div className="flex w-[680px] flex-col gap-[24px]">
            <h2 className="w-[680px] m-0 t-h2">Taking the bias away</h2>
          ...
          <aside className="flex w-[440px] flex-col gap-[12px] rounded-md bg-surface-panel p-[24px]">
            ...
            <p className="t-mono-lg whitespace-nowrap">{fmt(RAW)} − {fmt(BIAS)} = {fmt(COMPUTED)}</p>
            <p className="w-[392px] t-body-sm">
      ...
      <section className="flex w-full flex-col items-start gap-[40px] bg-surface-stage px-[120px] py-[48px]">
        <div className="flex flex-col gap-[12px]">
          ...
          <h2 className="w-[900px] m-0 t-h2 text-text-on-stage">Remove the bias</h2>
          <p className="w-[680px] t-body-md whitespace-pre-wrap text-text-on-stage-muted">{`The enlarged region shows the galaxy and the sky around it. Turn Bias on, then off again.  The bars underneath show the average pixel brightness in this region.`}</p>
        </div>
        <div className="flex flex-col gap-[16px]">
          <div className="flex items-center gap-[24px]">
            <h3 className="m-0 t-h3 whitespace-nowrap text-text-on-stage">Removing the bias</h3>
            <Toggle label="Bias" checked={bias} onChange={setBias} onStage />
          ...
          <div className="flex w-[1008px] flex-col gap-[12px] pt-[24px]">
            ...
            <div className="flex items-center gap-[8px]">
              ...
              <span className="t-label-sm tracking-[0.2px] whitespace-nowrap text-text-on-stage-muted">Bias share: {fmt(BIAS)} in this region</span>
            </div>
          </div>
        </div>
      </section>
