...
Try it, experiment 2 (the same light-frame ROI; the flat is on in both tiles; the 50 % flat, as
the Figma frames show):
  ...
  roi_exp2_removed.png         computed - raw, centred on mid grey, on its own symmetric stretch.
                               The difference is under 1 DN, so the span is the EXP2_SPAN_PERCENTILE
                               of |difference| (much tighter than experiment 1's) so the donut reads.
...
EXP2_SPAN_PERCENTILE = 98.0  # of |computed - raw| for experiment 2's removed tile; the difference is tiny
...
def main() -> None:
    ...
    # (the raw noise cancels because both tiles divide the same frame, so the stretch can be strong).
